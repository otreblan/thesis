...
For the Vulkan backend, Fdtd Lucuma implements the FDTD method using Vulkan\cite{vulkanspec} Compute Shaders written in Shader Slang\cite{10.1145/3197517.3201380}.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[process, below=of select] (services) {\blackcircle{1} Start basic services and selected backend. (As shown in Figure \ref{fig:fdtd_lucuma_service_graph})};
		\node[process, below=of services] (entity) {\blackcircle{2} Create a new ECS entity};
		\node[process, below=of entity] (simulation) {\blackcircle{3} Attach the simulation state and a private registry with the sources to \blackcircle{2}};
		\node[decision, below=of simulation, yshift=0.25cm] (while) {Can step?};


		\node[process, below=of while] (updateH) {\blackcircle{4} Update magnetic $\{x,y,z\}$ fields};
		\node[process, below=of updateH] (updateE) {\blackcircle{5} Update electric $\{x,y,z\}$ fields};
		\node[process, below=of updateE] (sources) {\blackcircle{6} Apply sources};
		\node[process, below=of sources] (abc) {\blackcircle{7} Apply boundary conditions};

		\node[process, right=2cm of while] (cleanup) {Cleanup \blackcircle{3}, \blackcircle{2} and \blackcircle{1}};
